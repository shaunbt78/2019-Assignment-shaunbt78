function getUsers(pageNumber){
    $.ajax({
        url: "http://127.0.0.1:5000/api/users?page="+ String(pageNumber),
        type: "GET",
...
function renderUsers(objUsers){
    ...
    var totalpgs = document.getElementById("totalPages")
    totalpgs = objUsers.total_pages
...
function nextPage(){
    ...
    var totalpgs = document.getElementById("totalPages")
    var pageCurrentInt = parseInt(pageCurrent.innerText)
    var totalpgsInt = parseInt(totalpgs.innerText)
    //console.log(totalpgsInt)
    if (pageCurrentInt < totalpgsInt){
    console.log("next")
    pageCurrentInt = pageCurrentInt+1
    pageCurrent.innerText = pageCurrentInt
    getUsers(pageCurrentInt)
    }
}
function prevPage(){
    var pageCurrent = document.getElementById("pageNumber")
...
function user1Get(){
    ...
    if (pageCurrent.innerText == "2"){
        ...
        $.ajax({
            url: "http://127.0.0.1:5000/api/users/"+ String(userID),
            type: "GET",
            success: function(response){
                //var objUsers = JSON.parse(response);
                userRender(response);
